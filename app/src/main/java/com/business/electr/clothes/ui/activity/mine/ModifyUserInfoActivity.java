...
public class ModifyUserInfoActivity extends BaseActivity<ModifyUserInfoPresenter> implements ModifyUserInfoView {
    ...
    public void getUserInfoSuccess(UserBean bean) {
        ...
        SynchronizationObserver.getInstance().onSynchronizationUpdate(SynchronizationObserver.TYPE_UPDATE_USER_INFO, userBean, SynchronizationObserver.PAGE_FRAGMENT_TYPE_MINE,SynchronizationObserver.PAGE_FRAGMENT_TYPE_ELECT);
        GlidUtils.setCircleGrid(this,ApiClient.BASE_URL + bean.getHeadImgUrl(),imgUploadPic);
        tvNickName.setText(bean.getPhone());
        etName.setText(bean.getNickName());
        etName.setSelection(bean.getNickName().length());
